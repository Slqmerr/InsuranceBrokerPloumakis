...
import type { NavProduct } from "../components/cmsProducts";
      ...
      {products.map((product) => {
        ...
        >
          <div style={{
            ...
            <Icon size={21} color="currentColor" strokeWidth={1.75} />
          </div>
          <div style={{
            fontSize: "16px",
            fontWeight: 600,
...
/** The interactive half of /asfaleies — search, expand/collapse, animations.
 *  Its product lists are read from the CMS by the server component in
 *  page.tsx, since this file is "use client" and can't read content/ itself. */
export default function AsfaleiesContent({
  idiotes,
  epixeirisi,
}: {
  idiotes: NavProduct[];
    ...
    <>
      <main style={{ background: "#f7f9fc", flex: 1 }}>
        <div className="page-shell" style={{ maxWidth: "1140px", margin: "0 auto", padding: "64px 36px 40px" }}>
          ...
          <CategorySection label="Ιδιώτες" products={idiotes} basePath="/idiotes" />
          <CategorySection label="Επιχειρήσεις" products={epixeirisi} basePath="/epixeirisi" />
        </div>
      </main>
      <Footer />
    </>
